...
users.forEach((user) => {  //<---rewritten
  emails.push(user.email); //<--you can have "return " before emails but not needed
});
console.log(emails);
...
// users.forEach ((user) => {  //<---rewritten // daynas way
//   names.push(user.name); //<--you can have "reutrn " before names but not needed
// });

users.forEach(user => names.push(user.name)); //<- daniels way
...
users.forEach((user) => { //rewritten
  ...
  const {name, email, languages} = user; //<---rewritten
  console.log(user);
